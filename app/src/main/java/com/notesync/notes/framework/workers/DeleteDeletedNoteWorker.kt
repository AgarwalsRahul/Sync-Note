...
class DeleteDeletedNoteWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        val result = safeApiCall(IO) {
            noteNetworkDataSource.deleteDeletedNote(
                newNote,
                user
            )
        }
        return when (result) {
            is ApiResult.Success -> {
                ...
                printLogD("DeleteDeletedNoteWorker", "Retrying request...")
                Result.retry()
    ...
    interface Factory : ChildWorkerFactory {
        override fun create(appContext: Context, params: WorkerParameters): DeleteDeletedNoteWorker
    }
}