...
class InsertDeletedNoteWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        val result = safeApiCall(IO) {
            noteNetworkDataSource.insertDeletedNote(
                newNote,
                user
            )
        }
        return when (result) {
            is ApiResult.Success -> {
                ...
                printLogD("InsertDeletedNoteWorker", "Retrying request...")
                Result.retry()
            }
    ...
    interface Factory : ChildWorkerFactory {
        override fun create(appContext: Context, params: WorkerParameters): InsertDeletedNoteWorker
    }
    }
